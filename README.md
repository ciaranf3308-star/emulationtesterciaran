...
## AUTHORITATIVE V3.1 – 2026-08-17 5.1.0 Notes Popular Rec Pulse Pinned Premium Motion (Higher Quality Than Pitched)

**Version jump 5.0.0 → 5.1.0** – magnitude loop engineering, no shortcuts, heavier frontend *intentionally* allowed because V3 zero-overhead handoff totally exits Crystal on PLAY (only tiny watcher remains). Previous V3 5.0.0 + V3.1 Track 7 content merged below. Machine-truth canonical (D:\CrystalFrontend\crystal-machine-config.json 19 systems ROM D:\Emulation\roms\ media D:\Emulation\storage\downloaded_media gamelist %APPDATA%\EmuDeck\ES-DE\gamelists writable D:\CrystalFrontend else LOCALAPPDATA correct build cargo tauri build --no-bundle) unchanged.

V3.1 Tracks landed at higher quality than suggestion:

1. **Game Notes** per-ROM 4000 char bounded, progress 0-100, UTC ISO, backup rolling 3, SAFE_MODE block, symlink/traversal reject, 256KB total, Rust+Tauri commands get_all/set/delete/get, frontend panel X→NOTES textarea slider Hold View quick memo overlay anywhere, surface progress on return – 337+231 lines panels, 380 line Rust 6 tests.
2. **Most Popular Per Console** 19×20 hand-curated titles no logos 380 titles, scoring playCount*0.6 + playTime + recency decay 10/(days+1)*1.8, Your Most Played rail 3-5 + curated fallback <5.
3. **Recommendations For You** Discover tabs For You/Trending/New, local scoring system affinity × genre × unplayed 1.6 bonus, seed favorites+last10, reason line "Because you loved X", no server ML.
4. **Emulation Pulse News** Rust cache D:\CrystalFrontend\cache\news\ 12h TTL 300KB prune, quick-xml RSS parse 4 feeds Libretro/EmuDeck/PCSX2/Dolphin 5s timeout, https whitelist host only, sanitize title<200 summary<500 dedup 20, safe_url_open rundll32 url.dll FileProtocolHandler CREATE_NO_WINDOW DETACHED, row bottom Discover.
5. **Collections Pinned Backlog** Rust writable_root+state/collections.json bounded <4KB secret reject, pinned max5 cross-system, backlog queued, Y+hold 600ms toggle Now Playing top rail, toggle safe, 5 tests.
6. **Visual Parallax+Glass+Spring+Aura (higher quality)** parallax 4-6px FG opposite 0.5× BG transform-only 60fps CSS vars --crystal-parallax-x/y, volumetric cyan shaft top-right radial-gradient blur 24px moves selection graphite/silver/cyan adherence liquid glass backdrop blur 12px border 1px rgba(255,255,255,0.08), spring carousel cubic-bezier 0.34,1.56,0.64,1 scale 1.08 centered 0.92 sides stagger --spring-index 40ms, aura idle 5s accent pulse 1.2× shadow PS2 blue GC purple token animation 2.2s ease.
7. **CRT+Sound+Haptic+Motion (higher quality)** CRT .crystal-crt-mode border-radius 3%/2% perspective 800px scale 1.02 scanline repeating-linear-gradient transparent 1px rgba 0.15 1px 2px vignette, toggle C/T/3 gamepad R-stick Settings General persisted crystal_crt_mode default off root class, motion stagger vars --crystal-bg-delay 0ms --crystal-hw-delay 80ms --crystal-game-delay 160ms reverse 0/40/80ms reduced-motion 120-160ms, micro sound WebAudio lazy 880Hz 60ms A 440Hz 80ms B volume 0.12 haptic 20ms vibrationActuator dual-rumble respects mute crystal_sounds_enabled default off, Settings General toggles OFF→ON tick.
8. **Heavier Fine Zero-Overhead** frontend 600kB 170 gzip 23.85 CSS due heavier visuals but lighter perf contract preserved never 1 video per row single hardware preload async visibilitychange pause prefers-reduced-motion transform/opacity worker threads long imports modal prevents dup extraction no PowerShell, watcher BEFORE terminate exit_after_handoff grace 2s duplicate guard sysinfo+lock 15s relaunch once --crystal-restored exit itself KEEP OPEN if creation fails SAFE_MODE blocks crash bounded <4KB.

Build: bun typecheck pass ✅ (tsc --noEmit --skipLibCheck + tsc -b projects ok), bun build 147 mods 600.54kB gzip170.20 CSS23.85 gzip5.51 9.32s ✅, cargo check 0 errors 101 warnings ✅, cargo test pending offline, bun test pending but discovery 132 prior.

Commit: `V3.1 – notes, popular, recommendations, pulse, pinned, premium motion parallax glass CRT aura` → push origin/main fast-forward clean. Full audits in docs/V3-AUDIT.md + docs/V3.1-AUDIT.md.
